...
const PAGE_HELP={
  ...
  employees:{t:'Сотрудники и доступы',d:'Кто допущен к системе: добавление сотрудников, их роли и контакты. Кнопка 🔑 у работника — персональные права: что он видит, изменяет, добавляет, удаляет по каждому разделу, плюс особые действия (оплата, счётчики, импорт, экспорт). Кнопка «🔑 Как настроить права» вверху — инструкция с примером. Внизу — общая матрица прав по ролям («✎ Изменить права»).'},
...
function help(){
  const steps=[
    ...
    'Внесите сотрудников и настройте им права — раздел «Сотрудники»: кнопка 🔑 у работника (видит / изменяет / добавляет / удаляет по каждому разделу).',
  ...
  ${isAdmin()?`<div class="card" style="margin-bottom:16px"><div class="sec-h" style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap"><span>🔑 Настройка прав сотрудников</span><button class="btn sm" onclick="permGuide()">Открыть инструкцию</button></div>
    <div class="t-sub" style="line-height:1.6">Что видит и что может менять каждый сотрудник — настраивается лично. Открывается в разделе «Сотрудники» кнопкой 🔑 у работника.</div></div>`:''}
...
/* Встроенная инструкция по настройке прав (то же, что в PDF для администратора) */
function permGuide(){
  const row=(a,b)=>`<tr><td class="t-strong" style="white-space:nowrap">${a}</td><td class="t-sub">${b}</td></tr>`;
  openM(`<div class="modal-h"><h3>🔑 Как настроить права сотруднику</h3><span class="x" onclick="closeM()">×</span></div>
  <div class="modal-b" style="line-height:1.6">
    <div class="doc" style="border-radius:11px;padding:11px 13px;margin-bottom:12px"><div style="flex:1;min-width:0">
      Раздел <b>«Сотрудники»</b> → кнопка <b>🔑</b> в строке работника. Роль (Управляющий, Бухгалтер, Отдел аренды, Эксплуатация) — это шаблон, а здесь доступ настраивается <b>лично для человека</b>. Дашборд соберётся автоматически: показываются только блоки тех разделов, которые ему открыты.
    </div></div>

    <div class="sec-h">Галочки в таблице</div>
    <table><tbody>
      ${row('👁 Видит','Раздел появляется в меню, данные приходят сотруднику. Без этой галочки раздела для него не существует — данные вообще не отправляются в его браузер.')}
      ${row('✎ Изменяет','Может править существующие записи.')}
      ${row('＋ Добавляет','Может создавать новые записи (кнопки «+ …»).')}
      ${row('🗑 Удаляет','Может удалять записи. Можно не давать — тогда испортить данные он не сможет.')}
    </tbody></table>
    <div class="t-sub" style="margin-top:8px">Система следит сама: изменять/добавлять/удалять нельзя без «Видит»; снимете «Изменяет» — «Добавляет» и «Удаляет» отключатся.</div>

    <div class="sec-h">Особые действия</div>
    <div class="t-sub">Отдельный блок под таблицей — разрешения «с деньгами и данными» внутри уже открытого раздела: вносить оплату аренды, начислять аренду, вносить показания счётчиков (котельная, ГСМ), выплачивать зарплату, импорт из файлов, экспорт CSV, синхронизация с банком и площадками.</div>

    <div class="sec-h">Пример: сотрудник по коммунальным услугам</div>
    <table><tbody>
      ${row('Видит','Объекты, Коммуналка и расходы, ФОТ, Заявки, ТО, Задачи')}
      ${row('Изменяет','Коммуналка, Заявки, ТО, Задачи')}
      ${row('Добавляет','Коммуналка, Заявки')}
      ${row('Удаляет','ничего')}
      ${row('Действия','только «Вносить показания счётчиков»')}
      ${row('Закрыто','Платежи аренды, Арендаторы, Договоры, Бюджет, Отчёты, Реклама, Синхронизация, Сотрудники')}
    </tbody></table>
    <div class="t-sub" style="margin-top:8px">На дашборде он увидит: заполняемость, структуру площадей, коммуналку (начислено / собрано / долг), просроченную коммуналку, расходы по категориям, ФОТ и центр сроков. Сбора аренды, долгов по аренде и бюджета — не увидит.</div>

    <div class="sec-h">После сохранения</div>
    <div class="t-sub">Нажмите «💾 Сохранить». Сотруднику нужно обновить страницу (Ctrl+F5, на Маке ⌘+Shift+R) или зайти заново. Кнопка «↺ Вернуть права роли» убирает персональную настройку. У администратора и собственника доступ всегда полный — ограничить нельзя. Все изменения прав пишутся в «Журнал действий».</div>
  </div>
  <div class="modal-f">${canView('employees')?`<button class="btn ghost" onclick="closeM();gotoPage('employees')">К сотрудникам</button>`:''}<div class="spacer"></div><button class="btn" onclick="closeM()">Понятно</button></div>`);
}
...
function employees(){
  ...
  el(head('Сотрудники и доступы',`${USERS.length} пользователей · роли и права доступа`,
    `${isAdmin()?`<button class="btn ghost" onclick="permGuide()">🔑 Как настроить права</button> `:''}${canAdd('employees')?`<button class="btn" onclick="userModal()">+ Сотрудник</button>`:''}`)+
...
function userPermModal(uid){
  ...
  <div class="modal-b">
    <div class="t-sub" style="margin-bottom:10px">Роль <b>${esc(u.roleTitle||u.role)}</b> — это шаблон. Здесь можно настроить доступ лично для этого сотрудника: что он <b>видит</b>, что может <b>изменять</b>, <b>добавлять</b> и <b>удалять</b>. Дашборд соберётся автоматически по этим правам. <a href="javascript:void(0)" onclick="permGuide()" style="color:var(--accent2)">ℹ️ Инструкция и пример</a></div>
