...
const STATE_MOD = { buildings:'objects', units:'objects', tenants:'tenants', contracts:'contracts',
  payments:'payments', utilities:'utilities', expenses:'utilities', buildingMeters:'utilities', heatCost:'utilities', fuelLog:'utilities', salaries:'salaries',
...
- ПЛОЩАДЬ (area): у помещений на плане обычно подписана площадь в м² (например «12,5» или «12.5 м²») — внимательно прочитай это число и укажи в area. Если у конкретного помещения число площади совсем не разобрать — поставь null (НЕ переноси площадь соседнего помещения). НЕ ставь всем помещениям одинаковую площадь и НЕ оценивай площадь «на глаз» по размеру — только реально прочитанное число;
...
function parsePlanUnits(raw){
  ...
  // анти-галлюцинация площади: если у 4+ помещений площадь одинаковая — модель почти наверняка
  // «выдумала» одно число (реальные площади так не совпадают). Лучше очистить, чтобы пользователь вписал сам.
  const nn = out.filter(x=>x.area!=null);
  if(nn.length>=4 && nn.every(x=>x.area===nn[0].area)) out.forEach(x=>{ x.area=null; });
  return out;
}
